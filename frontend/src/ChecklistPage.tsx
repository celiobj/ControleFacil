...
export default function ChecklistPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await api.get(`/properties/${propertyId}/checklist`)).data,
  });
  ...
  const update = useMutation({
    ...
    onSuccess: () =>
      client.invalidateQueries({ queryKey: ["checklist", propertyId] }),
  });
  const add = useMutation({
    mutationFn: () =>
      api.post(`/properties/${propertyId}/checklist/items`, form),
    onSuccess: () => {
      client.invalidateQueries({ queryKey: ["checklist", propertyId] });
      setForm(emptyItem);
      setOpen(false);
    ...
    onError: () => setFeedback("Não foi possível adicionar o item."),
  });
  ...
  return (
    <>
      <div className="section-heading">
        <div>
          <p className="eyebrow">CHECKLIST DO IMÓVEL</p>
          <h1>Acompanhamento</h1>
        </div>
        <div className="checklist-heading-actions">
          <span className="date-chip">
            {data?.progress?.percentage ?? 0}% concluído
          </span>
          <button
      ...
      {feedback && (
        <p className={feedback.includes("adicionado") ? "success" : "error"}>
          {feedback}
        </p>
      )}
      {open && (
        ...
        >
          ...
          <div className="form-grid">
            <label>
              ...
              >
                <option>ANTES DO LANCE</option>
                <option>ARREMATAÇÃO</option>
                <option>PÓS-ARREMATAÇÃO</option>
                <option>POSSE</option>
          ...
          <div className="form-actions">
            ...
            <button className="primary" disabled={add.isPending}>
              {add.isPending ? "Salvando..." : "Adicionar item"}
            </button>
          </div>
        </form>
      )}
      <div className="panel checklist-summary">
        <strong>
          {data?.progress?.completed ?? 0} de {data?.progress?.total ?? 0}
        </strong>
      ...
      <div className="panel checklist-table">
        <table>
          <thead>
            <tr>
              <th>Fase</th>
              <th>Etapa</th>
              <th>Tarefa / Verificação</th>
              <th>Responsável</th>
              <th>Prazo</th>
              <th>Status</th>
            </tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  <select
                    className={`check-status status-${item.status}`}
                    value={item.status}
                    onChange={(event) =>
